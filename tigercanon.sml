...
(*val canonizar = traceSchedule o basicBlocks o linearize*)
    
fun canonizar stm = 
    let
        val stms = linearize stm
        val (stmss, lab) = basicBlocks stms
        val stms2 = traceSchedule (stmss, lab)
    in
        (*print("Statements linearizados: \n");
        List.map (print o tigerit.tree) stms;
        print("Bloques basicos: \n");
        print(lab ^ "\n");
        List.app (List.app (print o tigerit.tree)) stmss;
        print("Staments finales: ");
        List.app (print o tigerit.tree) stms2;*)
        stms2
    end
end
